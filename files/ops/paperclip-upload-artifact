...
EOF
  if [[ -n "$body" ]]; then
    status_code="$(
      env -u PAPERCLIP_API_KEY -u PAPERCLIP_BRIDGE_API_KEY \
        /usr/bin/curl --config /dev/fd/3 -sS -X "$method" -w '%{http_code}' -o "$response_file" \
        "$url" \
...
EOF
  status_code="$(
    env -u PAPERCLIP_API_KEY -u PAPERCLIP_BRIDGE_API_KEY \
      /usr/bin/curl --config /dev/fd/3 -sS -X POST -w '%{http_code}' -o "$response_file" \
      "$url" \
...
case "$issue_id:$company_id" in
  *[!A-Za-z0-9:-]*)
    printf 'Issue and company ids may contain only letters, digits, and hyphens.\n' >&2
    exit 64
    ;;
esac
