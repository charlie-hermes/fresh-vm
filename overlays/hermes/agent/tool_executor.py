...
def _checkpoint_path_for_environment(resolved_path: str, task_id: str) -> str:
    """Map a container path back to an explicit host bind for snapshots."""
    try:
        ...
        container_path = PurePosixPath(resolved_path)
        for volume in config.get("docker_volumes", []):
            ...
            if len(parts) < 2:
                continue
            host_root, container_root = parts[0], parts[1]
            if not host_root or not container_root.startswith("/"):
                continue
            try:
                relative = container_path.relative_to(PurePosixPath(container_root))
            except ValueError:
                continue
            host_root_path = Path(host_root).expanduser().resolve()
            if host_root_path.is_dir():
                return str(host_root_path / Path(*relative.parts))
    except Exception:
        pass
    return resolved_path
...
def _ensure_file_checkpoint(
    ...
    checkpoint_path = _checkpoint_path_for_environment(
        str(resolved_path), effective_task_id or "default"
    )
    work_dir = agent._checkpoint_mgr.get_working_dir_for_path(checkpoint_path)
