...
def test_interrupt_stops_persistent_container_before_killing_exec(monkeypatch):
    env = docker_env.DockerEnvironment.__new__(docker_env.DockerEnvironment)
    env._container_id = "container-acceptance-123"
    env._docker_exe = "/usr/bin/docker"
    calls = []
    ...
    assert calls[0][0] == [
        "/usr/bin/docker", "stop", "-t", "1", "container-acceptance-123"
    ]
