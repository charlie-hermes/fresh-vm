...
def _make_env_config(**overrides):
    base = {
        ...
        "docker_env": {"STATIC": "value"},
        "docker_extra_args": ["--network=test-private"],
    }
...
class TestFileToolsContainerConfig:
    ...
    def test_docker_env_and_extra_args_passed(self):
        """File-tool containers receive the same Docker env and run flags as terminal."""
        cc = self._run(
            _make_env_config(
                docker_env={"STATIC": "value"},
                docker_extra_args=["--network=test-private", "--add-host=api:10.0.0.1"],
            ),
            "t-extra",
        ).get("container_config", {})
        assert cc.get("docker_env") == {"STATIC": "value"}
        assert cc.get("docker_extra_args") == ["--network=test-private", "--add-host=api:10.0.0.1"]
