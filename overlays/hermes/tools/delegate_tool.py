...
def _model_background_value(args: dict, parent_agent=None) -> bool:
    ...
    """
    # `hermes_local` is a one-shot child process. Detached work is interrupted
    # when that process exits, so Paperclip explicitly opts into synchronous
    # parent delivery while the batch itself still runs children concurrently.
    # Interactive Hermes sessions retain the upstream background behaviour.
    if is_truthy_value(os.getenv("HERMES_FORCE_SYNC_DELEGATION"), default=False):
        return False
