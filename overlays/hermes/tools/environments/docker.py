...
def _get_active_profile_name() -> str:
    """Return a stable container-reuse identity for the active Hermes home.

    Named profiles already have a unique profile name. Arbitrary external
    ``HERMES_HOME`` paths are reported upstream as ``default`` or ``custom``;
    hashing the resolved explicit home prevents two Paperclip agents with the
    same task id from reusing a container with the other agent's mounts.
    """
    try:
    ...
    try:
        resolved_home = str(Path(configured_home).expanduser().resolve())
    except (OSError, RuntimeError):
        resolved_home = os.path.abspath(os.path.expanduser(configured_home))

    import hashlib

    suffix = hashlib.sha256(resolved_home.encode("utf-8")).hexdigest()[:16]
    return f"{profile_name}-{suffix}"
...
class DockerEnvironment(BaseEnvironment):
    ...
    def _kill_process(self, proc):
        """Stop the sandbox before killing its local ``docker exec`` client.

        Killing only the client does not terminate the exec process inside a
        persistent container. A Paperclip timeout would therefore leave the
        command running after Hermes exited. Stopping the container is the
        fail-closed boundary: bind-mounted state remains, and the normal reuse
        path starts the stopped container on the next invocation.
        """
        container_id = self._container_id
        if container_id:
            try:
                subprocess.run(
                    [self._docker_exe, "stop", "-t", "1", container_id],
                    capture_output=True,
                    text=True,
                    timeout=10,
                    check=False,
                ...
                logger.warning(
                    "Failed to stop Docker sandbox %s during interruption: %s",
                    container_id[:12],
                    exc,
                )
