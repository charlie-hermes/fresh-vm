...
# The ChatGPT Codex Responses backend reserves the literal `web_search` name
# and does not expose that client-side function to the model. Keep the
# upstream name for every compatible provider and add a collision-safe alias
# that dispatches through the exact same configured backend.
WEB_SEARCH_HERMES_SCHEMA = {
    "name": "web_search_hermes",
    "description": "Search the web through Hermes' configured backend. This is the collision-safe alias for web_search on provider surfaces that reserve that name.",
...
registry.register(
    name="web_search_hermes",
    toolset="web",
    schema=WEB_SEARCH_HERMES_SCHEMA,
    handler=lambda args, **kw: web_search_tool(args.get("query", ""), limit=args.get("limit", 5)),
    check_fn=check_web_api_key,
    requires_env=_web_requires_env(),
    emoji="🔍",
