...
const HERMES_DEFAULT_PROMPT_TEMPLATE = [
    ...
    "- Use the mounted `paperclip-api` client from the terminal; browser/web tools may not reach the private API.",
    "- The client reads `$PAPERCLIP_API_URL`, the short-lived `$PAPERCLIP_API_KEY`, and `$PAPERCLIP_RUN_ID` without exposing bearer values in process arguments.",
    "- Never use raw `curl -H` with a Paperclip bearer token, print identity variables, hard-code local ports, or copy secrets into files or comments.",
    "- `paperclip-api` automatically adds the run header and fails closed if an agent mutation lacks `$PAPERCLIP_RUN_ID`.",
    "- Read the `paperclip-employee` skill before coordinating tasks, checkout, delegation, interactions, approvals, or artifacts.",
    "- For multiline JSON, build the payload with `jq` and pipe it to `paperclip-api METHOD /path -`.",
    "",
    ...
    "  paperclip-api PATCH /issues/{{context.issueId}} -",
    "```",
    "",
...
export async function execute(ctx) {
    ...
    const configuredPaperclipApiUrl = cfgString(config.paperclipApiUrl);
    if (configuredPaperclipApiUrl)
        env.PAPERCLIP_API_URL = configuredPaperclipApiUrl;
    if (ctx.runId)
