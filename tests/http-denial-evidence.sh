...
valid="[12:34:56] \033[33mWARN\033[39m: \033[36mPOST /api/companies/$company/issues?acceptanceProbe=$probe 403\033[39m"

printf '%b\n' "$valid" | "$check" "$company" "$probe" >/dev/null
for invalid in \
  "POST /companies/$company/issues?acceptanceProbe=$probe 403" \
  "POST /api/companies/$company/issues?acceptanceProbe=$probe 400" \
  "POST /api/companies/$company/issues?acceptanceProbe=other 403"
do
